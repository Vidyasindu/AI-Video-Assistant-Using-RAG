...
# # We use session_state to persist data across reruns (like pipeline results and chat history).
# # To this:
...
# def run_ui_pipeline(source: str, language: str) -> dict:
#     """Runs the pipeline while updating Streamlit status indicators."""
#     status_text = st.empty()
#     progress_bar = st.progress(0)
    
#     try:
#         status_text.text("⏳ Step 1/5: Processing video/audio input...")
#         chunks = process_input(source)
#         progress_bar.progress(20)
        
#         status_text.text("⏳ Step 2/5: Transcribing audio (this may take a moment)...")
#         transcript = transcribe_all(chunks, language)
#         progress_bar.progress(50)
        
#         status_text.text("⏳ Step 3/5: Generating title and summary...")
#         title = generate_title(transcript)
#         summary = summarize(transcript)
#         progress_bar.progress(70)
        
#         status_text.text("⏳ Step 4/5: Extracting insights, action items, and decisions...")
#         action_item = extract_action_items(transcript)
#         decisions = extract_key_decisions(transcript)
#         questions = extract_questions(transcript)
#         progress_bar.progress(90)
        
#         status_text.text("⏳ Step 5/5: Building conversational RAG engine...")
...
#     st.markdown("Provide your meeting/video source below to kickstart the analysis.")
    
#     source_input = st.text_input(
#         "Source URL or File Path", 
#         placeholder="e.g., https://www.youtube.com/watch?... or C:/video.mp4"
#     )
...
# st.markdown("Transform your videos into structured summaries, key deliverables, and an interactive chatbot.")
# st.markdown("---")

# # Handle Processing Action
# if process_btn:
#     if not source_input.strip():
#         st.warning("⚠️ Please provide a valid YouTube URL or local file path first.")
#     else:
#         with st.spinner("Analyzing media... please do not close this tab."):
#             result = run_ui_pipeline(source_input.strip(), language_input)
#             if result:
#                 st.session_state.pipeline_result = result
#                 st.session_state.chat_history = [] # Reset chat for new file
#                 st.success("🎉 Processing completed successfully!")

# # 3. Main Panel Dashboard Content
# if st.session_state.pipeline_result:
#     res = st.session_state.pipeline_result
    
#     # Display the dynamic title generated by your model
#     st.header(f"📌 {res['title']}")
    
#     # Split layout into two sections: Analysis Dashboard & Dynamic Q&A Chat
#     col_dash, col_chat = st.columns([1.1, 0.9], gap="large")
    
#     # Left Column: Insights & Content Dashboard
#     with col_dash:
#         st.subheader("📊 Extracted Insights")
        
#         tab_summary, tab_actions, tab_decisions, tab_questions, tab_transcript = st.tabs([
#             "📋 Summary", "✅ Action Items", "🔑 Key Decisions", "❓ Open Questions", "📝 Raw Transcript"
#         ])
...
#         with tab_questions:
#             st.markdown("### Remaining Questions")
#             st.markdown(res["open_questions"])
            
#         with tab_transcript:
#             st.markdown("### Complete Raw Transcript")
#             st.text_area("Full text:", value=res["transcript"], height=400, disabled=True)

#     # Right Column: The RAG Chat Engine Interface
#     with col_chat:
#         st.subheader("💬 Chat with your Video")
#         st.caption("Ask questions about specific moments, timelines, statements, or overall context.")
        
#         # Create a container box with fixed height to keep chat scrollable and tidy
#         chat_container = st.container(height=450)
        
#         # Render historical chat interactions from session state
#         with chat_container:
#             for message in st.session_state.chat_history:
#                 with st.chat_message(message["role"]):
#                     st.markdown(message["content"])
        
#         # Accept real-time user chat text inputs
#         if user_query := st.chat_input("Ask something about this session..."):
#             # Display user's question right away in the UI
#             with chat_container:
...
#             # Fetch response from RAG Chain
#             with st.spinner("Assistant is thinking..."):
#                 answer = ask_question(res["rag_chain"], user_query)
            
#             # Display assistant's response inside container
#             with chat_container:
#                 with st.chat_message("assistant"):
#                     st.markdown(answer)
                    
#             # Save assistant message to context history
#             st.session_state.chat_history.append({"role": "assistant", "content": answer})

# else:
#     # Landing View placeholder before file input processing runs
#     st.info("👈 Enter a media source in the sidebar menu and click **'Process Video'** to extract metrics and open the chat companion.")
